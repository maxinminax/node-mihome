...
module.exports = class MiioDevice extends EventEmitter {
  ...
  async init() {
    if (this._miotSpecType) {
      await this.miotFetchSpec(this._miotSpec);
    }
  ...
  async send(method, params, options = {}) {
    const result = this.protocol === 'cloud' ? await this.localSend(method, params, options) : this.cloudSend(method, params, options);
    return result;
  }
  ...
  async miioCall(method, params, options = {}) {
    ...
    if (options.refresh !== false) {
      await sleep(50);
      await this._loadProperties(options.refresh);
    }
