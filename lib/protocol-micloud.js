...
class MiCloudProtocol {
  ...
  async getDevices(deviceIds, country = 'cn') {
    const req = deviceIds ? {
      dids: deviceIds,
    } : {
    ...
    const data = await this.request('/home/device_list', req, country);

    return data.result.list;
  }

  async getDevice(deviceId, country = 'cn') {
    const data = await this.request('/home/device_list', {
  ...
  async miioCall(deviceId, method, params) {
    const data = await this.request(`/home/rpc/${deviceId}`, { method, params });
    return data.result;
  }
