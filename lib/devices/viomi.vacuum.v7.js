...
module.exports = class extends Device {
  ...
  getSideBrushRemaining() {
    const sideBrushUsed = parseInt(this.properties.side_brush_hours, 10);
    const sideBrushLife = parseInt(this.properties.side_brush_life, 10);

    if (sideBrushUsed >= 0 && sideBrushLife) {
      return Math.max(((sideBrushLife - sideBrushUsed) / sideBrushLife) * 100, 0);
    }
    return undefined;
  }

  getMainBrushRemaining() {
    const mainBrushUsed = parseInt(this.properties.main_brush_hours, 10);
    const mainBrushLife = parseInt(this.properties.main_brush_life, 10);

    if (mainBrushUsed >= 0 && mainBrushLife) {
      return Math.max(((mainBrushLife - mainBrushUsed) / mainBrushLife) * 100, 0);
    }
    return undefined;
  }

  getHypaRemaining() {
    const hypaUsed = parseInt(this.properties.hypa_hours, 10);
    const hypaLife = parseInt(this.properties.hypa_life, 10);

    if (hypaUsed >= 0 && hypaLife) {
      return Math.max(((hypaLife - hypaUsed) / hypaLife) * 100, 0);
    }
    return undefined;
  }

  getMopRemaining() {
    const mopUsed = parseInt(this.properties.mop_hours, 10);
    const mopLife = parseInt(this.properties.mop_life, 10);

    if (mopUsed >= 0 && mopLife) {
      return Math.max(((mopLife - mopUsed) / mopLife) * 100, 0);
    }
    return undefined;
  }
